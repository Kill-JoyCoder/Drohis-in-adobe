...
# Load expected schema for validation
...
def validate_against_schema(data, schema):
    # A minimal recursive validation
...
# def extract_outline_json(pdf_path, json_path, max_pages=50):
#     doc = fitz.open(pdf_path)
#     metadata_title = doc.metadata.get("title", "").strip()
#     title = metadata_title or ""
...
#         first_page = doc[0]
#         spans = [span for block in first_page.get_text("dict")["blocks"]
#                    if "lines" in block for line in block["lines"]
#                    for span in line["spans"]]
#         if spans:
#             big_span = max(spans, key=lambda x: x["size"])
#             title = big_span["text"].strip()
...
# if __name__ == "__main__":
#     result = extract_outline_json("C:/Users/Asus/hackathons/Drohis-in-adobe/Task-1A/input/file01.py", "output.json")

# so far tried tried the task, cannoot read into the pdf: